...
void Console::build() {
	entry_box = Moveable();
	entry_box.setColour(40, 40, 40, 250);
	...
	entry_text = Text(0.30, 0.475, Fonts::getFont(CONSOLAS_BOLD, 30), 189, 195, 199, 175, "");
	entry_text.addFlag(DISABLED);

	debug_box.setColour(20, 20, 20, 250);
	...
	debug_text = Text(0.30, 0.55, Fonts::getFont(CONSOLAS_BOLD, 14), 22, 160, 133, 175, "");
	debug_text.addFlag(DISABLED);

	feedback_text = Text(0.30, 0.595, Fonts::getFont(CONSOLAS_BOLD, 14), 26, 188, 156, 175, "");
	...
	try {
		if (args[0] == "FPSLIMIT") {
			...
			if (game->selected_object) {
				...
				game->selected_object->setVelocity(game->selected_object->getVelocity()[0], stod(args[1]));
