...
typedef map<string, Mapping>(*MappingFunction)(void*);
...
class UIManager {
	...
	public:
		...
		static map<string, Mapping> MapProvince(void* moveable);
		static map<string, Mapping> MapNation(void* moveable);
		static map<string, Mapping> MapUnit(void* moveable);
		static map<string, Mapping> MapWarDeclaration(void* war_details);
};
