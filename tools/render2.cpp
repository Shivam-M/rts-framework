...
void Render::drawText(string text, double x, double y, font_data font, double* colour) {
    ...
    glfwGetWindowSize(window_, &resolution[0], &resolution[1]);
...
void Render::drawCircle(Circle* c) {
    if (c->getFlags() & RING)
        ...
        // if (c->getFlags() & BACKGROUND) drawCircle(c->getLocation()[0] - ((*objects_)[0]->getLocation()[0] * 0.03), c->getLocation()[1], c->getRadius(), c->getGLColour(), c->getMatrix());
        drawCircle(c->getLocation(), c->getRadius(), c->getGLColour(), c->getGLGradient(), c->getGenerality());
...
void Render::drawCircle(double x, double y, double radius, double* colour, double* gradient_colour, double generality) {
    ...
    glfwGetWindowSize(window_, &dimensions[0], &dimensions[1]);
...
void Render::drawQuad(double x, double y, double width, double height, double* colour, double* gradient_colour) {
    ...
    if (render_method == 0) {
        ...
        colour = colour == nullptr ? new double[4] {1.0, 1.0, 1.0, 1.0} : colour;
...
void Render::drawCurvedQuad(double x, double y, float width, float height, float radius, double* colour, double* gradient_colour) {
    ...
    colour = colour ? colour : new double[4] {1.0, 1.0, 1.0, 1.0};
...
void Render::drawTexture(Moveable* m) {
    if (m->getFlags() & PLAYER) {
        ...
        drawTexture(m->getLocation(), m->getSize(), m->getTexture(), m->getGLColour(), true);
    }
}
...
void Render::drawTexture(double x, double y, double width, double height, Texture* texture, double* colour, bool right) {
    ...
    glTexCoord2i(t1, 1); glVertex2d(x, y);
    glTexCoord2i(t1, 0); glVertex2d(x, y + height);
    glTexCoord2i(!t1, 0); glVertex2d(x + width, y + height);
    glTexCoord2i(!t1, 1); glVertex2d(x + width, y);
...
void Render::renderWindow() {
    ...
    for (Moveable* m : *objects_) {
        if (m->getFlags() & CIRCLE) drawCircle((Circle*)m);
